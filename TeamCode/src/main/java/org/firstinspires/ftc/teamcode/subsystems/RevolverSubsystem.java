...
public class RevolverSubsystem {
    ...
    private static final double COLOR_DETECT_DISTANCE_CM = 4.0;
    ...
    public void update() {
        ...
        switch (currentState) {
            case LOADING:
                ...
                // Reset shoot requests to prevent "Runaway" rotation where it immediately goes to shooter
                ...
                if (inventory[currentIntakeSlot] == SlotColor.EMPTY) {
                    if (intakeEnabled) intake.setPower(INTAKE_POWER);
    ...
    private int findNextEmptySlot(int current) {
        for (int i = 1; i <= 3; i++) {
            ...
            if (inventory[next] == SlotColor.EMPTY) return next;
        }
        return -1; // Full
    }
    ...
    public RevolverState getState() { return currentState; }
    
    public String getInventoryString() {
        return Arrays.toString(inventory);
    }
    
    public int getIntakeSlot() { return currentIntakeSlot; }
    
    private SlotColor determineColor(double dist, int r, int g, int b) {
        if (dist > COLOR_DETECT_DISTANCE_CM) return SlotColor.EMPTY;
        if (r < 50 && g < 50 && b < 50) return SlotColor.EMPTY; 
        if (g > (r + 20) && g > (b + 20)) return SlotColor.GREEN;
        else if (r > g || b > g || b > r) return SlotColor.PURPLE;
        return SlotColor.UNKNOWN;
    }
    
    // Command Variables for Telemetry
    public SlotColor getCurrentColor() { return lastDetectedColor; } 
    ...
    public long getTargetSlot() { return globalTargetTicks / TICKS_PER_SLOT; } // Approx
    public int getIndexerCurrentPos() { return indexer.getCurrentPosition(); }
    public long getIndexerTargetPos() { return globalTargetTicks; }
